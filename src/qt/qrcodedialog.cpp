    ...
    address(addr)
{
    ...
    if (label.trimmed().isEmpty()) {
        setWindowTitle(QString("OrbitalCoin - Generate QR Code"));
    } else {
        setWindowTitle(QString("OrbitalCoin - Generate QR Code for %1").arg(label));
    }
...
{
    QString ret = QString("orbitalcoin:%1").arg(address);
    int paramCount = 0;
